...
const int Classic4Cutter::defPrefPieces = 150;
const double Classic4Cutter::widthToHeightRatio = 1.0;

QString Classic4Cutter::toString() const { return getName(); }
QString Classic4Cutter::getName() const { return QObject::tr("Classic-4"); }
QString Classic4Cutter::getDescription() const {
    return QObject::tr("Creates pieces where each side is equally likely to have a hole or"
                       " a knob; each piece may have 0-4 holes, and will tend to have 2."
                       " Each piece is roughly square, not counting holes and knobs.");
}

Classic4Cutter::Classic4Cutter() {
    setPreferredPieceCount(defPrefPieces);
}

Classic4Cutter::Classic4Cutter(int prefPieces) {
    setPreferredPieceCount(prefPieces);
}

void Classic4Cutter::setPreferredPieceCount(int count) {
    if (count < 2) throw IllegalArgumentException(QString("Invalid preferred pieces: ").arg(count));
    prefPieces = count;
}

void Classic4Cutter::writeExternal(QDataStream * out) {
    *out << prefPieces;
}

void Classic4Cutter::readExternal(QDataStream * in) {
    *in >> prefPieces;
}

const QList<Piece *> Classic4Cutter::cut(const QImage & image) {
    ...
    for (int j = 0; j < rows; j++) {
      for (int i = 0; i < columns-1; i++) {
        ...
        if (_rand() >= 0.5) { QPoint temp = p1; p1=p2; p2=temp; }
...
Piece * Classic4Cutter::makePiece(const QImage & image,
